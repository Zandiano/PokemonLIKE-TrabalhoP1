...
bool CatchEntity(struct player *catcher, struct entity catch, int currentHeart, int slot, int diff, bool cheatMode){
    if(rand()%100 > catchEq(currentHeart, catch.level-catcher->level, catch.specie.rarity, diff) && !cheatMode)
        return FALSE;
